...
Testing is difficult due to a problem that came up during implementation of the swap prototype. The \ac{EVM} handles funds sent by a smart contract as internal transactions, which do not show up on the recipients account balance. Information who owns the coins after sending it is only available in the contract's state. Making them visible requires tracing the \ac{EVM}'s stack on assembly level to keep track on who owns the funds.
